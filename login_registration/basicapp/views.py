...
def register(request):
    ...
    if request.method == "POST":
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            usernamegot=userObj['username']
            emailgot=userObj['email']

            if not (User.objects.filter(username=usernamegot).exists() or User.objects.filter(email=emailgot).exists()):
...
def activate(request,uidb64,token): #creating a function for activating emailID
    ...
    if user is not None and account_activation_token.check_token(user,token):# if user is none and token is checked via tokens.property
        ...
        return render(request,"basicapp/successfull_registration.html")
